...
export class LocalPlayerController {
    ...
    startSlide(path, duration, onComplete) {
    ...
    update(dt) {
        if (this.disposed)
            return this.getState();
        if (this.slideActive)
            return this.updateSlide(dt);
        let inputX = this.stickX;
        let inputZ = this.stickZ;
        if (this.codes.has('KeyW') || this.codes.has('ArrowUp'))
            inputZ += 1;
        if (this.codes.has('KeyS') || this.codes.has('ArrowDown'))
            inputZ -= 1;
        if (this.codes.has('KeyA') || this.codes.has('ArrowLeft'))
            inputX -= 1;
        if (this.codes.has('KeyD') || this.codes.has('ArrowRight'))
            inputX += 1;
        const running = this.sprintHeld || this.codes.has('ShiftLeft') || this.codes.has('ShiftRight');
        ...
        if (this.hVel.lengthSquared() < 0.002)
            this.hVel.set(0, 0, 0);
        ...
        const disp = new Vector3(this.hVel.x * dt, walkingOnFloor ? 0 : this.vy * dt, this.hVel.z * dt);
        ...
        if (!this.grounded)
            this.animation = this.vy > 1.2 ? 'jump' : 'fall';
        else if (moving)
            this.animation = running && inputLen > 0.2 ? 'run' : 'walk';
        else
            this.animation = 'idle';
    ...
    isWalkableFloor(name) {
        if (name === 'ground' || name === 'grass-ground' || name === 'plaza-pavement')
            return true;
        if (/^path-\d+$/.test(name))
            return true;
        if (name === 'playground-slide-deck' ||
            name === 'playground-slide-landing' ||
            name === 'playground-slide-stair-ramp') {
            return true;
        }
        if (name === 'playground-trampoline-pad')
            return true;
        if (name.startsWith('playground-slide-step-'))
            return true;
    ...
    refreshGrounded() {
        ...
        const hit = this.scene.pickWithRay(new Ray(origin, Vector3.Down(), 2.1), (mesh) => {
            if (!mesh || mesh === this.root)
                return false;
            if (mesh.name.startsWith('local-player'))
                return false;
