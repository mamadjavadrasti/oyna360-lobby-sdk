...
export class LobbyMusic {
    ...
    async unlock(autoStartMusic = false) {
        ...
        try {
            ...
            if (autoStartMusic && !this.skipAutoStart)
    ...
    tone(ctx, dest, freq, time, dur, type, vol) {
    ...
    thump(ctx, dest, freq, time, dur, vol) {
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        osc.type = 'sine';
        osc.frequency.setValueAtTime(freq, time);
    ...
    sweep(ctx, dest, from, to, time, dur, vol) {
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        osc.type = 'triangle';
    ...
    noiseBurst(ctx, dest, time, dur, vol, freq) {
        if (!this.noise)
            return;
...
//# sourceMappingURL=lobby-music.js.map