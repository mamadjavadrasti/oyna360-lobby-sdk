...
function isWalkThroughVisual(name) {
    if (!name || name.includes('nametag') || name.includes('-sign'))
        return true;
    if (name === 'local-player')
        return false;
    if (name.startsWith('local-player'))
        return true;
    if (/^path-\d+$/.test(name))
        return true;
    return [
...
function ghostMaterial(scene) {
    let mat = scene.getMaterialByName('lobby-phys-ghost');
    if (!mat) {
...
function stampCollider(mesh, scene) {
...
function ensureCylinder(scene, name, x, z, diameter, height, y) {
...
function ensureBox(scene, name, x, y, z, width, height, depth, rotationY) {
    let block = scene.getMeshByName(name);
    if (!block) {
...
export function syncCharacterObstacle(scene, rootName, x, z) {
    ensureCylinder(scene, `${rootName}-charcol`, x, z, 0.95, 1.95, 0.97);
}
export function removeCharacterObstacle(scene, rootName) {
 ...
 */
export function applyLobbyCollisions(scene) {
    ...
    for (const mesh of snapshot) {
        ...
        if (/^path-\d+$/.test(n)) {
            ...
            for (const [side, s] of [
                ...
                ensureBox(scene, `${n}-rail-${side}`, p.x + sx * s * 1.32, 0.95, p.z + sz * s * 1.32, 0.62, 1.9, depth, yaw);
            }
        }
        if (/^tree-\d+$/.test(n)) {
...
export function isSolidLobbyMesh(mesh) {
    const n = mesh.name;
    if (!n || isWalkThroughVisual(n))
        return false;
    return (n === 'ground' ||
        n === 'fountain-blocker' ||
        n.endsWith('-phys') ||
...
//# sourceMappingURL=lobby-colliders.js.map