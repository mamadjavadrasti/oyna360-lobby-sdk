...
import { captureWebGlInfo, diagSetAlwaysSelectAsActiveMesh, diagSetAvatarDoubleSide, diagSetDisableUniformBuffers, getLobbyPerfPhaseSummary, resetLobbyPerfDiag, sampleAvatarRender, setLobbyPerfDiagEnabled, } from './lobby-perf-diag';
function sleep(ms) {
...
export function captureRealDeviceInfo(engine) {
    const ua = typeof navigator !== 'undefined' ? navigator.userAgent : '';
    const canvas = engine.getRenderingCanvas?.() ?? null;
    const webgl = captureWebGlInfo(engine);
    const nav = typeof navigator !== 'undefined' ? navigator : null;
    const babylonVersion = Engine.VERSION ??
        engine.constructor?.EngineVersion ??
        'unknown';
    let hardwareScalingLevel = null;
    try {
    ...
    return {
        ...
        fpsCapNote: 'If display is 60Hz, browser may cap FPS near 60; prefer frame time / scene_render / p95 / p99 / max over absolute FPS.',
    };
}
function avatarPlayer(userId, glbUrl, presetKey, position, animation = 'walk') {
    return {
        userId,
...
function layoutPos(layout, i, count) {
    if (layout === 'far_front') {
        const col = i % 5;
    ...
    if (layout === 'mixed') {
        if (i % 3 === 0)
            return layoutPos('near_front', i, count);
        if (i % 3 === 1)
            return layoutPos('far_front', Math.floor(i / 2), count);
...
function startWalkDriver(bridge, ids, layout) {
...
async function ensureRemoteCount(bridge, liveIds, needRemotes, glbUrl, presetKey, idPrefix) {
    ...
    while (liveIds.length < needRemotes) {
        ...
        bridge.upsertRemote(avatarPlayer(userId, glbUrl, presetKey, layoutPos('near_front', n, needRemotes), 'walk'));
        const ok = await bridge.waitRemoteReady(userId, 90_000);
        if (!ok)
            throw new Error(`remote ready timeout: ${userId}`);
        liveIds.push(userId);
    }
}
async function measureJoin(bridge, scene, engine, player, label, sampleMs) {
 ...
 */
export async function runRealDeviceAvatarSuite(bridge, options = {}) {
    ...
    if (!cachedA)
        notes.push(`${baseA.id} not cached before scale — results may include load cost`);
    ...
    for (const total of [1, 2, 4, 8, 10]) {
        try {
            ...
            stop();
            const phases = getLobbyPerfPhaseSummary();
            const misses = (phases.recentMarks || []).filter((m) => m.phase === 'glb_container_load' && String(m.detail || '').includes('cache_miss'));
    ...
    await sleep(300);
    report.player2Join.A_solo = compact(await sampleAvatarRender(scene, engine, 'join_A_solo', joinSampleMs));
    report.player2Join.B_sameBase = await measureJoin(bridge, scene, engine, avatarPlayer('rd-join-same', baseA.glbUrl, baseA.id, { x: 2, y: 0, z: 4 }, 'walk'), 'join_B_same', joinSampleMs);
    bridge.removeRemote('rd-join-same');
    await sleep(300);
    // C: different base — deliberately NOT preloaded so join hitch is measurable
    report.player2Join.C_differentBaseCold = await measureJoin(bridge, scene, engine, avatarPlayer('rd-join-diff', baseB.glbUrl, baseB.id, { x: -2, y: 0, z: 4 }, 'walk'), 'join_C_diff_cold', joinSampleMs);
    ...
    report.player2Join.C2_differentBaseWarm = await measureJoin(bridge, scene, engine, avatarPlayer('rd-join-diff2', baseB.glbUrl, baseB.id, { x: -2.5, y: 0, z: 4 }, 'walk'), 'join_C2_diff_warm', joinSampleMs);
    ...
    if (!options.skipFrustum) {
        ...
        for (const layout of ['near_front', 'far_front', 'behind', 'mixed']) {
            ...
            await sleep(400);
            report.frustum[layout] = compact(await sampleAvatarRender(scene, engine, `frustum_${layout}_n8`, sampleMs));
            stop();
        }
    }
    else {
    ...
    if (!options.skipAb) {
        ...
        }
        {
            ...
            const stop = startWalkDriver(bridge, liveIds, 'near_front');
            await sleep(350);
            report.ab.alwaysSelectOff = compact(await sampleAvatarRender(scene, engine, 'ab_alwaysSelect_OFF', sampleMs));
            stop();
            diagSetAlwaysSelectAsActiveMesh(scene, true);
        }
        {
            ...
            const stop = startWalkDriver(bridge, liveIds, 'near_front');
            await sleep(350);
            report.ab.doubleSideOff = compact(await sampleAvatarRender(scene, engine, 'ab_doubleSide_OFF', sampleMs));
            stop();
            diagSetAvatarDoubleSide(scene, true);
        }
        {
            ...
            const stop = startWalkDriver(bridge, liveIds, 'near_front');
            await sleep(350);
            report.ab.uboEnabled = compact(await sampleAvatarRender(scene, engine, 'ab_ubo_ENABLED', sampleMs));
            stop();
            diagSetDisableUniformBuffers(engine, true);
        }
    }
...
export function attachRealDeviceAvatarDiag(bridge, handle, win = window) {
...
//# sourceMappingURL=lobby-real-device-diag.js.map