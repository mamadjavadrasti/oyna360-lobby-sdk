 ...
 */
import { BoundingInfo, Mesh, TransformNode, VertexBuffer, VertexData, } from '@babylonjs/core';
 ...
 */
function prepareGlbSkeleton(input) {
    const { scene, name, glbUrl, model, importRoot, rootScaleZ, meshes, transformNodes, skeletons, animationGroups, } = input;
    const hasSkeleton = skeletons.length > 0 || meshes.some((m) => !!m.skeleton);
    let clips = animationGroups;
    let displayMeshes = meshes;
    let rigidGlb = false;
...
function resolveLimbPivots(scene, name, visual, limbRig) {
...
function buildHumanoidAfterFit(visual, skeletons, transformNodes, boneDriven) {
...
export class AvatarRigBuilder {
    static prepareGlbSkeleton(input) {
        return prepareGlbSkeleton(input);
    }
    static padSkinnedBounds(meshes) {
        padSkinnedBounds(meshes);
    }
    static preferBoneTexture(skeletons) {
        preferBoneTexture(skeletons);
    }
    static bindSkeletonRig(skeletons, transformNodes) {
        return bindSkeletonRig(skeletons, transformNodes);
    }
    static buildHumanoidAfterFit(visual, skeletons, transformNodes, boneDriven) {
        return buildHumanoidAfterFit(visual, skeletons, transformNodes, boneDriven);
    }
    static resolveLimbPivots(scene, name, visual, limbRig) {
    ...
    static build(instance) {
        if (instance.rig !== undefined)
            return instance.rig;
        return instance.root.metadata?.rig ?? null;
    }
}
//# sourceMappingURL=avatar-rig-builder.js.map