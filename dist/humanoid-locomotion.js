...
export const HUMANOID_JOINTS = Object.keys(HUMANOID_JOINT_INDEX);
...
export class HumanoidPose {
    ...
    angles = new Float32Array(HUMANOID_JOINT_COUNT * 3);
    ...
    add(joint, pitch, yaw, roll, weight = 1) {
        if (weight === 0)
            return;
...
const GAIT_KEYS = Object.keys(WALK_GAIT);
/** Cross-fade walk → run in place (same phase, so the blend never snaps). */
export function lerpGait(out, a, b, t) {
...
export function cloneGait(source) {
    return { ...source };
}
/**
 ...
 */
function writeLeg(pose, thigh, shin, foot, theta, g, weight) {
    ...
    const kneeFlex = g.kneeSwing * Math.pow(swingPhase, 1.35) + g.kneeStance * (0.55 + 0.45 * stancePhase);
 ...
 */
function writeArm(pose, shoulder, upperArm, foreArm, hand, theta, inward, g, weight) {
    const s = Math.sin(theta);
 ...
 */
export function writeGaitPose(pose, phase, g, weight) {
    if (weight <= 0)
        return;
...
export function writeIdlePose(pose, time, weight) {
    if (weight <= 0)
        return;
...
export function writeAirPose(pose, rising, weight) {
    if (weight <= 0)
        return;
    if (rising) {
...
export function writeSlidePose(pose, weight) {
    if (weight <= 0)
        return;
 ...
 */
export function gaitFrequency(speed, referenceSpeed, gait) {
...
export function footContacts(prevPhase, advance) {
    if (advance <= 0)
        return 0;
...
//# sourceMappingURL=humanoid-locomotion.js.map