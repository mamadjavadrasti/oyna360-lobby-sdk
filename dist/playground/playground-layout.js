...
export function slideLocalToWorld(local) {
...
function mat(scene, id, hex, emissive = 0, alpha = 1) {
...
export function slideLocalPath() {
...
function buildSlide(scene, lobby) {
    ...
    for (const side of [-1, 1]) {
        ...
        solidMesh(chuteRail);
        const stairRail = MeshBuilder.CreateCylinder(`playground-slide-stair-rail-${side}`, { diameter: 0.09, height: stairHyp, tessellation: 8 }, scene);
...
function createStairWalkRamp(scene, bottomY, topY, bottomZ, topZ) {
    const left = [];
    const right = [];
    const samples = 20;
    ...
    const ramp = MeshBuilder.CreateRibbon('playground-slide-stair-ramp', { pathArray: [left, right], closeArray: false, closePath: false, updatable: false, sideOrientation: Mesh.DOUBLESIDE }, scene);
    ramp.isVisible = false;
    ramp.isPickable = true;
    return ramp;
}
function createSlideChute(scene) {
    ...
    for (let i = 0; i <= samples; i++) {
        ...
        if (right.lengthSquared() < 1e-6)
            right = Vector3.Cross(tangent, new Vector3(1, 0, 0));
        right.normalize();
    ...
    const chute = MeshBuilder.CreateRibbon('playground-slide-bed', { pathArray, closeArray: false, closePath: false, updatable: false, sideOrientation: Mesh.DOUBLESIDE }, scene);
    return chute;
}
function playgroundLabel(scene, id, text, color) {
...
function buildTrampoline(scene, lobby) {
...
//# sourceMappingURL=playground-layout.js.map