import { BoundingInfo, Color3, Material, Mesh, MeshBuilder, SceneLoader, StandardMaterial, Texture, TransformNode, Vector3 as BVector3, VertexBuffer, VertexData, } from '@babylonjs/core';
...
import { buildHumanoidBoneRig } from './humanoid-rig';
...
function readTintFromConfig(config) {
    if (!config)
        return undefined;
...
function trimDetailMapsOnMaterials(meshes) {
    for (const mesh of meshes) {
 ...
 */
function trimAvatarMapsForLowEndDevices(scene, meshes) {
...
function excludeAvatarFromGlow(scene, meshes) {
...
function padSkinnedBounds(meshes) {
    for (const mesh of meshes) {
...
function preferBoneTexture(skeletons) {
        ...
        if (hasSkeleton) {
            ...
            if (limbRig) {
                ...
                for (const mesh of displayMeshes) {
                    if (mesh.skeleton)
                        mesh.numBoneInfluencers = Math.min(mesh.numBoneInfluencers || 4, 4);
                }
                boneDriven = true;
