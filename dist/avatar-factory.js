import { Color3, Mesh, MeshBuilder, StandardMaterial, TransformNode, Vector3 as BVector3, VertexBuffer, } from '@babylonjs/core';
...
import { AvatarMaterialPipeline, applyAvatarBaseAlbedo, applyAvatarTint, hardenAvatarMaterials, lobbyQualityOf, readBaseAlbedoUrlFromConfig, readTintFromConfig, trimDetailMapsOnMaterials, } from './avatar-material-pipeline';
import { AvatarRigBuilder } from './avatar-rig-builder';
...
export class AvatarFactory {
        ...
        const run = async () => {
            ...
            if (!options.skipAccessories) {
                try {
                    ...
                    await attachAccessoriesFromAvatarConfig(scene, root, avatar.customConfig);
            ...
            const baseAlbedo = readBaseAlbedoUrlFromConfig(avatar.customConfig);
            ...
            if (!baseAlbedo) {
                applyAvatarTint(root, readTintFromConfig(avatar.customConfig));
            }
            else {
    ...
    static markLobbyAvatar(root, scene) {
    ...
    static syncCrowdBudget(scene) {
        ...
        scene.metadata?.applyLobbyCrowdLoad?.(count);
    ...
    static async createFromGlb(scene, glbUrl, name, displayName, username, options) {
        ...
        const importRoot = result.meshes.find((m) => isImportRootName(m.name)) ??
        ...
        const { clips, displayMeshes, rigidGlb, boneDriven, limbRig, originalAnimationGroups, skeletons: preparedSkeletons, transformNodes: preparedTransformNodes, } = prepared;
        ...
        await yieldToRenderLoop();
        const humanoid = AvatarRigBuilder.buildHumanoidAfterFit(visual, preparedSkeletons, preparedTransformNodes, boneDriven);
        ...
        const { torso, head, armL, armR, legL, legR } = AvatarRigBuilder.resolveLimbPivots(scene, name, visual, limbRig);
        if (!options.skipNameTag) {
            attachNameTag(scene, visual, displayName ?? '', username ?? '', name);
        }
        let collider = null;
     ...
     */
    static createPlaceholderInstance(scene, avatar, name, displayName, username, options = {}) {
     ...
     */
    static async createInstanceAsync(scene, avatar, name = 'avatar', displayName, username, options = {}) {
