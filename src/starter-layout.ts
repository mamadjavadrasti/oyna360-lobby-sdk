...
export interface StarterLayoutConfig {
  ...
  gamePortals?: Array<{ slug: string; name: string; position: Vec3 }>;
}

export function applyStarterLayout(lobby: PlatformLobby, config: StarterLayoutConfig = {}) {
  const scene = lobby.getScene();
  const {
    showZones = true,
    showGameplayPortal = true,
    showGrid = true,
    gamePortals = [],
  } = config;
...
export function createPortalVisual(
  ...
  const ring = MeshBuilder.CreateTorus(
    `portal-ring-${id}`,
    { diameter: 3.2, thickness: 0.28, tessellation: 48 },
    scene,
  );
  ring.parent = root;
  ring.position.y = 2.2;
