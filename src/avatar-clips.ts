...
export const AVATAR_CLIP_ALIASES: Record<LobbyAnimationState, string[]> = {
...
export type NamedClip = { name: string };

function matchAlias<T extends NamedClip>(clips: readonly T[], aliases: string[]): T | null {
 ...
 */
export function pickAvatarClip<T extends NamedClip>(
  clips: readonly T[],
  state: LobbyAnimationState,
): T | null {
...
export function dedupeClipsByName<T extends NamedClip>(clips: readonly T[]): T[] {
