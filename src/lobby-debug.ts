...
export function setLobbyCameraPreset(lobby: PlatformLobby, preset: LobbyCameraPreset) {
...
export function buildLobbyDebugReport(lobby: PlatformLobby): LobbyDebugReport {
  ...
  const checks: LobbyDebugCheck[] = [
    ...
    {
      name: 'room_pads',
      pass: ['dual', 'reborn', 'space', 'tower', 'racing', 'zombie', 'survival'].every((id) =>
        meshSet.has(`room-pad-${id}`),
      ),
    },
    ...
    (() => {
      ...
      for (const node of nodes) {
        ...
        if (verts < 24 || !mesh.skeleton || base === '__root__' || base === 'world') continue;
        skinned++;
...
export interface LobbyDebugHandle {
  ...
  getLocalPose: () => { position: { x: number; y: number; z: number }; rotationY: number; animation: string };
...
export function attachLobbyDebug(lobby: PlatformLobby, win: Window = window): LobbyDebugHandle {
  const handle: LobbyDebugHandle = {
    ready: true,
