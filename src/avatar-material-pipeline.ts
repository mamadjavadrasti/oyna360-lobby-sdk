...
import type { AbstractMesh, BaseTexture, GlowLayer } from '@babylonjs/core';
import {
  isTouchDevice,
  ...
  for (const mesh of meshes) {
    for (const mat of collectAlbedoMaterials(mesh.material)) {
      ...
      if (absolute && scene) {
        ...
        reassertMaterialOpaque(mat as Material);
        ...
        if ('albedoTexture' in mat && meta.baseAlbedoTexture !== undefined) {
          ...
          forceOpaqueTexture(mat.albedoTexture, mat as Material);
        }
        if ('diffuseTexture' in mat && meta.baseDiffuseTexture !== undefined) {
          mat.diffuseTexture = meta.baseDiffuseTexture;
          forceOpaqueTexture(mat.diffuseTexture, mat as Material);
        }
        reassertMaterialOpaque(mat as Material);
      }
      mat.markAsDirty?.();
...
function reassertMaterialOpaque(mat: Material) {
  mat.alpha = 1;
...
function reassertOpaqueOnAvatarRoot(root: TransformNode) {
  ...
  for (const mesh of meshes) {
    if (!mesh || mesh.name?.includes('nametag')) continue;
    if ((mesh.metadata as { isNameTag?: boolean } | undefined)?.isNameTag) continue;
    for (const mat of collectAlbedoMaterials(mesh.material)) {
      reassertMaterialOpaque(mat);
      forceOpaqueTexture(
        (mat as AlbedoSwapMaterial).albedoTexture ?? (mat as AlbedoSwapMaterial).diffuseTexture,
        mat,
      );
    }
  }
...
function forceOpaqueTexture(tex: BaseTexture | null | undefined, mat?: Material) {
  ...
  const lockMat = (m: Material) => {
    m.alpha = 1;
    m.transparencyMode = Material.MATERIAL_OPAQUE;
    m.alphaMode = 0;
    m.disableDepthWrite = false;
    (m as Material & { useAlphaFromAlbedoTexture?: boolean }).useAlphaFromAlbedoTexture = false;
    (m as Material & { useAlphaFromDiffuseTexture?: boolean }).useAlphaFromDiffuseTexture = false;
  ...
  type OpaqueMeta = {
    lobbyForceOpaque?: boolean;
    lobbyOpaqueMatBound?: WeakSet<Material>;
  };
  const meta = (tex.metadata ?? {}) as OpaqueMeta;
  const boundMats = meta.lobbyOpaqueMatBound ?? new WeakSet<Material>();
 ...
 */
function ensureAvatarGlowOcclusion(scene: Scene, meshes: AbstractMesh[]) {
  const glow = (scene.metadata as { plazaGlow?: PlazaGlow; plazaGlowAvatarOccludeInstalled?: boolean } | undefined)
    ?.plazaGlow;
  if (!glow) return;

  const meta = (scene.metadata ?? {}) as {
    plazaGlow?: PlazaGlow;
    plazaGlowAvatarOccludeInstalled?: boolean;
  };

  if (!meta.plazaGlowAvatarOccludeInstalled) {
    glow.customEmissiveTextureSelector = ((mesh, _subMesh, material) => {
      if (mesh.metadata?.isLobbyAvatar) return null as unknown as Texture;
      return (
        (material as Material & { emissiveTexture?: Texture | null })?.emissiveTexture ??
        (null as unknown as Texture)
      );
    }) as GlowLayer['customEmissiveTextureSelector'];
    glow.customEmissiveColorSelector = (mesh, _subMesh, material, result) => {
      ...
      if (mat?.emissiveColor) {
        ...
        result.set(
          mat.emissiveColor.r * textureLevel,
          mat.emissiveColor.g * textureLevel,
          mat.emissiveColor.b * textureLevel,
          mat.alpha,
        );
      } else {
        result.set(0, 0, 0, 1);
