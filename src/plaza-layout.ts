...
export interface PlazaLayoutConfig {
  ...
  onRoomStart?: (room: { id: string; name: string; gameSlug?: string }) => void;
}

/** Default rooms for the starter plaza only. Games should pass their own `rooms`. */
export const DEFAULT_PLAZA_ROOMS: PlazaRoomDef[] = [
...
export function applyPlazaLayout(lobby: PlatformLobby, config: PlazaLayoutConfig = {}) {
...
function mat(scene: Scene, id: string, hex: string, emissive = 0) {
...
function makeEntranceWalkway(scene: Scene, i: number) {
...
function buildFountain(scene: Scene) {
...
function buildShops(scene: Scene, lobby: PlatformLobby) {
  const shops = [
...
function plantNature(scene: Scene) {
  for (let i = 0; i < 18; i++) {
...
function makeTree(scene: Scene, id: string, x: number, z: number, scale: number) {
...
function makeBush(scene: Scene, id: string, x: number, z: number) {
...
function makeLamp(scene: Scene, i: number, x: number, z: number) {
...
function makeBench(scene: Scene, i: number, x: number, z: number, rot: number) {
...
function buildRoom(scene: Scene, room: PlazaRoomDef): RoomRuntime {
...
function decorateTheme(scene: Scene, root: TransformNode, room: PlazaRoomDef) {
...
function labelMaterial(scene: Scene, id: string, texture: DynamicTexture) {
  const m = new StandardMaterial(id, scene);
...
function makeRoomSign(scene: Scene, room: PlazaRoomDef) {
...
function paintSign(texture: DynamicTexture, room: PlazaRoomDef, occupants: number, status: string) {
...
function makeSign(scene: Scene, id: string, text: string, color: string) {
...
function spawnNpc(scene: Scene, pos: Vec3, preset: (typeof NPC_PRESETS)[number], id: string) {
  const npc = AvatarFactory.create(
    scene,
    {
      presetId: id,
      presetKey: 'npc',
...
function spawnPlazaCrowd(scene: Scene, count: number) {
  for (let i = 0; i < count; i++) {
    ...
    spawnNpc(
      ...
      NPC_PRESETS[i % NPC_PRESETS.length],
      `plaza-npc-${i}`,
    );
  }
}

function joinRoom(lobby: PlatformLobby, runtime: RoomRuntime, all: RoomRuntime[]) {
  if (runtime.occupants < runtime.def.maxPlayers) runtime.occupants += 1;
...
function leaveRoom(lobby: PlatformLobby, runtime: RoomRuntime, all: RoomRuntime[]) {
...
function serializeRooms(all: RoomRuntime[]) {
  return {
    rooms: all.map((r) => ({
      id: r.def.id,
      name: r.def.name,
...
function animateScene(
  scene: Scene,
  runtimes: RoomRuntime[],
  lobby: PlatformLobby,
  onRoomStart?: PlazaLayoutConfig['onRoomStart'],
) {
  let t = 0;
  const lastCeil: Record<string, number> = {};
  scene.registerBeforeRender(() => {
    ...
    for (const r of runtimes) {
      ...
      if (veil) veil.rotation.y = t * 0.8;
      const portal = scene.getMeshByName(`room-portal-${r.def.id}`);
      if (portal) portal.rotation.z = Math.sin(t) * 0.08;
      if (r.countdown != null) {
        r.countdown -= dt;
...
function runtimeSign(r: RoomRuntime) {
  return r.texture;
}
