...
export function attachLobbyPresenceUi(lobby: PlatformLobby): () => void {
  ...
  const push = (text: string, kind: 'join' | 'leave') => {
    ...
    root.prepend(item);
    while (root.childElementCount > MAX_ITEMS) root.lastElementChild?.remove();
  ...
  return () => {
    ...
    offJoin();
    offLeave();
    if (root.dataset.lobbyPresenceDynamic === '1') root.remove();
    else root.replaceChildren();
  };
}

function ensurePresenceRoot(lobby: PlatformLobby): HTMLElement {
...
function injectPresenceKeyframes() {
  if (document.getElementById('oyna-lobby-presence-style')) return;
